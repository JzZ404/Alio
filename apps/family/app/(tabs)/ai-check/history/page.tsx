...
import { AI_CHAT_HISTORY } from '@alio/mock-data';

/**
 * Family AI chat history — the family-side counterpart of the caregiver Logs
 * history (Figma 254:2429). Same header and row layout; each row is a past
 * conversation with Alio rather than a visit log.
 */
export default function AIChatHistoryPage({ onBack }: { onBack?: () => void } = {}) {
  const router = useRouter();
  const back = onBack ?? (() => router.push('/ai-check'));
  ...
  return (
    ...
    >
      ...
      <header className="absolute left-[25px] right-[25px] top-[60px] z-10 flex items-center justify-between">
        ...
        <IconBox size={42} aria-label="Search chat history">
      ...
      <div className="absolute bottom-[110px] left-0 right-0 top-[128px] overflow-y-auto px-5 py-3">
        <ul className="flex flex-col gap-2.5">
          {AI_CHAT_HISTORY.map((item) => (
            <li key={item.id}>
              <LogListItem name={item.patientName} date={item.date} />
            </li>
          ))}
        </ul>
      </div>
